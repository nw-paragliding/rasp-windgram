...
###On August 18, 2015, NCEP will discontinue the following data dissemination services:
...
###http://ftp.ncep.noaa.gov
  ...
  #4test:
 `echo "ARGS= @ARGV" >| tmptmp.gribftpget.args`;
  #4test: `echo "gribgetftptimeoutsec = $gribgetftptimeoutsec" >> tmptmp.gribftpget.args`;
  `echo "url=${gribftpsite}/${FTPDIRECTORY}/${filename}" >> tmptmp.gribftpget.args`;
#####################################33
####:cat tmptmp.gribftpget.args 
###  ARGS= PNW,ETA,18Z+9,SUN 12 Jul 2015,http://nomads.ncep.noaa.gov,pub/data/nccf/com/nam/prod/nam.20150712,nam.t18z.awip3d09.tm00.grib2,/home/tj/drjack/RASP/RUN/ETA/gribftpget.stdout,/home/tj/drjack/RASP/RUN/ETA/gribftpget.stderr,/home/tj/drjack/RASP/RUN,/home/tj/drjack/RASP/RUN/ETA/GRIB,/home/tj/drjack/UTIL,180,900,5000000,/home/tj/drjack/RASP/RUN/./rasp.pnw.printout
...
############################################
  ...
  my $program = "$ENV{PWD}/gribftpget.pl";   # used in email message
  ...
  #old $logfile = "${GRIBDIR}/gribftpget.log.${ifile}";
  ...
  ############################################
    ...
    ### SET CURL RETURN CODE CASES : leading * => forces error exit (else loop continues)
    ...
    $errormsg[8]   = "PARSE ERROR 8";           ###  once due to server being temporarily down 
    ...
    $errormsg[78] = "REMOTE GRIB FILE NOT FOUND $filename";
    $errormsg[127]  = "* CURL PROGRAM FAILURE";
    $errormsg[247]  = "* ID/PW LOGIN FAILURE";
    $errormsg[256] = "* GRIBFTPGET RC 256";   ### RC=256 error - but dont know what it means but seems severe!
  ...
  {  
    ...
    `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} >> GRIBFTPGET LOOP TOP: $filename" >> $logfile`;

    ### LOG GRIB DOWNLOAD START
    ### use file for two fields to match fields written by log_grib_download_size
    ( $hhmm = $time ) =~ s/:..$//;
    `echo "GRIBFTPGET  $rundayprt $hhmm $hhmm ${filename} url=${gribftpsite}/${FTPDIRECTORY}/${filename}        0" >> "$RUNDIR/$MODELTYPE/download.log"`;
    #old `echo "GRIBFTPGET  $rundayprt $hhmm $hhmm ${filename}         0" >> "$GRIBDIR/download.log"`;
    $gribftpgetreturn = 'precall';           # set here to avoid possible undef value in print

#DUMFTP-TESTING: call dummy ftp program for testing
#DUMFTP-TESTING+ print $PRINTFH "CALL FOR FAKE FTP FOR $ifile = THEN EXIT LOOP \n";
#DUMFTP-TESTING+   $ltimelimiterr = &timelimitexec ( $gribgetftptimeoutsec, "\$gribftpgetreturn=`$UTILDIR/xgribftpget $gribftpsite $GRIBDIR $FTPDIRECTORY $filename ${GRIBFTPSTDOUT}.${ifile} ${GRIBFTPSTDERR}.${ifile} $ftpoptions`" );
#DUMFTP-TESTING+ last;
...
#old  $ltimelimiterr = &timelimitexec ( $gribgetftptimeoutsec, "\$gribftpgetreturn=`$UTILDIR/gribftpget $gribftpsite $GRIBDIR $FTPDIRECTORY $filename ${GRIBFTPSTDOUT}.${ifile} ${GRIBFTPSTDERR}.${ifile} $ftpoptions`" );

############################################################################
######################  START OF FORMER BASH SCRIPT  #######################
    
      #example $UTILDIR/gribftpget $gribftpsite $GRIBDIR $FTPDIRECTORY $filename ${GRIBFTPSTDOUT}.${ifile} ${GRIBFTPSTDERR}.${ifile} $ftpoptions

      ### GET GRIB FILE yymmddzulu & WRITE TO GRIBDIR/file
      ### 7 arguments
      #   gribftpget SITE locdlDIR gribfiledirectory gribfilename std.out.filename err.out.filename ftp-options
      #   eg: gribftpget SITE /users/glendeni/SOAR/BLIP maps_fcst 0203721000006.grib
      #       "$DIR/tmp.${program}.gribftpget.out1" "$DIR/tmp.${program}.gribftpget.out2" -C -
      ...
      if    ( $gribftpsite eq "140.90.33.41" )
      { $ID='anonymous wxtofly@comcast.net'; }
      elsif ( $gribftpsite  eq "140.90.33.31")
          { $ID='anonymous wxtofly@comcast.net'; }
      elsif ( $gribftpsite  eq "http://nomads.ncep.noaa.gov")
          { $ID=''; }
      elsif ( $gribftpsite  eq "https://nomads.ncep.noaa.gov")
          { $ID=''; }
      elsif ( $gribftpsite  eq "nomads.ncep.noaa.gov")
          { $ID=''; }
      elsif ( $gribftpsite  eq "ftp-ftpprd-woc.lb.noaa.gov")
          { $ID='anonymous wxtofly@comcast.net'; }
      ...
      {
        ### gribftpget ERROR: bad site = $gribftpsite"
        exit -9 ;  
      ...
      ### CURL CASE TREATED HERE  tjo Change ftp to http for nomads 
      elsif ( $LSFTPMETHOD eq "CURL" )
      {
        $ID =~ s/ /:/;
	`echo  "$UTILDIR/curl -v -s -f $ftpoptions  -o ${GRIBDIR}/${filename} ${gribftpsite}/${FTPDIRECTORY}/${filename}" > $logfile`;
        ...
        #verbose:
           `$UTILDIR/curl -v -s -f $ftpoptions -o "${GRIBDIR}/${filename}" "${gribftpsite}/${FTPDIRECTORY}/${filename}" > $TMPFILE1 2>$TMPFILE2`;
	  ## remove user and ftpoptions and disable-epsv for http
          ##`$UTILDIR/curl -v -s  -o "${GRIBDIR}/${filename}" "http://${gribftpsite}/${FTPDIRECTORY}/${filename}" > $TMPFILE1 2>$TMPFILE2`;
      }
...
###TTTESTING `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> *GETGRIB-POST OPTIONS= $ftpoptions ==RETURN=${gribftpgetreturn}==" >> $logfile`;

    #why was i formerly calling rucfptls after gribftpget here ?
    #old $ltimelimiterr = &timelimitexec ( $gribgetftptimeoutsec, "`$UTILDIR/gribftpget $gribftpsite $GRIBDIR $filename $GRIBFTPSTDOUT $GRIBFTPSTDERR`; &gribftpls($gribftpsite);" );

    ### ASSUME THAT RETURN CODE IS "wait" TYPE CONFLATION OF 2 NUMBERS
    if ( $gribftpgetreturn !~ m|^\s*$| )
    {
      $exit_value  = $gribftpgetreturn >> 8 ;    #=int($?/256)
      $signal_num  = $gribftpgetreturn & 127 ;   #=($?-256*exit_value#)
    }
    else
    {
      $exit_value = 0;
    ...
    if ( $signal_num > 0 )
    #old if ( $gribftpgetreturn !~ m|^\s*$| && $signal_num > 0 )
    {
      `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> ** GRIBFTPGET RC ERROR (?SIGNAL?) EXIT **  RC= $gribftpgetreturn => $exit_value & $signal_num" >> $logfile`;
      exit $gribftpgetreturn ;
    }
    ...
    if ( $exit_value > 0 )
    {
      if ( defined $errormsg[$exit_value] )
      {
        ...
        {
          `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> ** GRIBFTPGET ERROR EXIT *${errormsg[$exit_value]} ** RC= $gribftpgetreturn => $exit_value & $signal_num" >> $logfile`;
          exit $gribftpgetreturn ;
        }
        ...
        {
          `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> GRIBFTPGET LOOP WARNING - $errormsg[$exit_value] - CONTINUE AFTER $cycle_waitsec sec" >> $logfile`; 
          sleep $cycle_waitsec;
          next;
        }
      }
      ...
      else
      {
        ...
        {
          ### send mail to warn me of what has occurred
          $subject = "**GRIBFTPGET.PL CHILDFTP UNKNOWN RETURN CODE";
          `echo "From ${program}: $time $MODELTYPE $ifile ${gribftpgetreturn} => $exit_value & $signal_num
  *** PLEASE REPORT THIS ERROR TO DRJACK SO A FIX-UP CAN BE PUT INTO THE SCRIPT
  also note whether RASP was continued successfully  despite this error or if the error caused RASP to fail" | mail -s "$subject" "$emailaddress"`;
        ...
        #2continue:
        `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> *NOTICE* GRIBFTPGET UNEXPECTED RC = $gribftpgetreturn => $exit_value & $signal_num - NO ACTION TAKEN " >> $logfile`;
        #2exit:        `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> **ERROR** GRIBFTPGET UNEXPECTED RC = $gribftpgetreturn => $exit_value & $signal_num - SCRIPT EXITED " >> $logfile`;
        exit $gribftpgetreturn ;
      }
    ...
    { 
      ...
      `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> GRIBFTPGET LOOP - LOCAL GRIB FILE NOT FOUND: likely ftp error - return= ${gribftpgetreturn} => $exit_value & $signal_num - CONTINUE AFTER $cycle_waitsec sec" >> $logfile`; 
      sleep $cycle_waitsec;
      next;
     }
    ...
    if ( defined $localgribfilesize )
      ...
      if ( $remotegribfilesize ne '' && $localgribfilesize < $remotegribfilesize )
      {
       ...
       `cp ${TMPFILE2} "${TMPFILE2}.getgrib_gribtoosmall1"`;
        ...
        `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> GRIBFTPGET LOOP - DOWNLOADED GRIB FILE TOO SMALL: $filename local $localgribfilesize < remote $remotegribfilesize - CONTINUE AFTER  $getgrib_waitsec sec" >> $logfile`; 
    ...
    else
    {
        ...
        ### SET ATTEMPT RE-TRY CHOICE
        `echo "-> $ifile CHILDFTP $$ ${iwait}/${getgrib_maxwait} -->> GRIBFTPGET LOOP ** STAT FAILED ** for $filename - CONTINUE AFTER $getgrib_waitsec sec" >> $logfile`; 
        sleep $getgrib_waitsec;
        next;
    }
  ...
  `echo "-> $ifile CHILDFTP $$ $time ${iwait}/${getgrib_maxwait} -->> GRIBFRPGET END **WARNING** max.iters $getgrib_maxwait reached -  lastRC=${gribftpgetreturn} !" >> $logfile`;
   exit -2 ;    # 'GLOOP' exit
