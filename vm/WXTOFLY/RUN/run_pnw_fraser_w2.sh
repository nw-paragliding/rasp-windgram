...
#Nested window RASP run for FRASER
if $WXTOFLY_RUN/run_rasp_nested.sh "PNW${N}" "FRASER${N}" "$WXTOFLY_RUN/PARAMETERS/rasp.run.parameters.FRASER${N}.${INIT}z" ; 
then
	$WXTOFLY_RUN/run_background_task.sh $WXTOFLY_RUN/run_tasks_w2.sh "FRASER${N}"
else
