...
#dis 
#***********************************************
#* Modifications by Xin-Zhong Liang 12/13/05    *
#*                                              *
#*  Illinois State Water Survey, MC-674         *
...
#                                       is used)
#        enddate =  YYYYMMDDHH format, overwriting -l fcstlen if set
#
...
require 5;
use strict;
use vars qw($opt_c $opt_h $opt_i $opt_d $opt_e $opt_f $opt_s $opt_l 
...
if ($opt_h){
    print "grib_prep.pl Usage
          ...
          -e YYYYMMDDHH
             Cycle time to use for endding time.  If set, then the
             -l argument is ignored

          -f filter
             Used to set a Perl-syntax pattern-match filter to
             apply when looking for GRIB file names. Exclude perl's 
             expression delimiter slash / marks on each side.  E.g. 
             -f avn (to find avn* analysis files) or 
             -f CY.00[0-1][0-9] (to find the first 19 hours of a model 
             output labeled CY.00*).  
          ...
          -s YYYYMMDDHH
             ...
             uses the real-time clock in conjunction with variables in
...
Need one command line argument, for example:\n
     grib_prep.pl source
     source is model name (AVN or ETA or NRA or RII);
\n"};
...
#jack: THIS RESULTS IN SPECIFIED INPUT HOUR BEING IGNORED SO REMOVE IT !!!
#jack-   if ($startdate gt $latest_avail) {
#jack-     print "Setting startdate ($startdate) to latest_avail ($latest_avail) 
#jack- based on actual current time.\n";
#jack-     $startdate = $latest_avail;
#jack-   }
...
# Get ending time.
my $fcstlen;
if (! $opt_e) { # Use -l if end-time not set
  if (defined $opt_l){
    $fcstlen = $opt_l;
  }else{
    print "Using default forecast length of 36 hours.\n";
...
if (defined $opt_e) { # Compute forecast length
  $fcstlen = (&wrfsi_utils::compute_interval($startdate, $enddate))/3600;
  if ($fcstlen <= 0){
    print "Enddate specified is before Startdate!\n";
    exit; }
  print "Using -e to set forecast length of $fcstlen hours.\n";
}
...
# --------------------------------------------------------------------------
#
# Time info needed for Liang's additions
#
my $year_start = ${$namehash{START_YEAR}}[0];
my $month_start = ${$namehash{START_MONTH}}[0];
my $day_start = ${$namehash{START_DAY}}[0];
my $hour_start = ${$namehash{START_HOUR}}[0];

my $year_end = ${$namehash{END_YEAR}}[0];
my $month_end = ${$namehash{END_MONTH}}[0];
my $day_end = ${$namehash{END_DAY}}[0];
my $hour_end = ${$namehash{END_HOUR}}[0];

# --------------------------------------------------------------------------
#
# Liang's NCEP/NCAR Reanalysis file naming convention (ending with yymmddhh)
#         these include pgb.f00yymmddhh (3D) and grb2dyymmddhh (2D)
#

if ($ngribfiles == 0) {      ########################### big if till the end

my $NRA_start_filter = $year_start.$month_start.$day_start.$hour_start;
my $NRA_end_filter = $year_end.$month_end.$day_end.$hour_end;
#**** the following is to account for grb2d files which is 6-hr forecast
#     but recorded in the file name at the initial time (-6h)
my $NRA_start_filter_2d = &wrfsi_utils::compute_time ($NRA_start_filter, -6);
my $NRA_end_filter_2d = &wrfsi_utils::compute_time ($NRA_end_filter, -6);
my $NRA_pattern_2d = "grb2d";
my $yyyy = 0;
my $foundNRA = 0;
  ...
  foreach (@allfiles) {
    if (/(\d\d)(\d\d\d\d\d\d)$/) {
      $foundNRA = 1;
      my $yy = $1; my $mmddhh = $2;
#     **** y2k problem introduced by the NRA convention
      if ($yy > 20) {
	  $yyyy = $yy + 1900; }
      else {
          $yyyy = $yy + 2000; }
      my $this_time = $yyyy.$mmddhh;
      if (/^$NRA_pattern_2d/) {
          if ( ($this_time >= $NRA_start_filter_2d) &&
               ($this_time <= $NRA_end_filter_2d) ) {
               @gribfiles = (@gribfiles,$_);
          }
      }
      else {
          if ( ($this_time >= $NRA_start_filter) &&
               ($this_time <= $NRA_end_filter) ) {
               @gribfiles = (@gribfiles,$_);
          }
      }
    }
  ...
  if ($foundNRA == 1) {
    if ($ngribfiles == 0) {
      my $msg = "FAILURE: Found NRA-type files but nothing during $NRA_start_filter - $NRA_end_filter";
      print "$msg\n";
      print LOG "$msg\n";
      #exit;
    }
    else {
      my $msg = "Found NRA-type files during $NRA_start_filter - $NRA_end_filter";
      print "$msg\n";
      print LOG "$msg\n";
    }
  }
...
#
# Liang's NCEP/DOE Reanalysis-2 file naming convention (ending with yyyymmddhh.grib)
#         these include pgb.anl.yyyymmddhh.grib (3D) and flx.ft06.yyyymmddhh.grib (2D)
#

if ($ngribfiles == 0) {
my $RII_start_filter = $year_start.$month_start.$day_start.$hour_start;
my $RII_end_filter = $year_end.$month_end.$day_end.$hour_end;
#**** the following is to account for grb2d files which is 6-hr forecast
#     but recorded in the file name at the initial time (-6h)
my $RII_start_filter_2d = &wrfsi_utils::compute_time ($RII_start_filter, -6);
my $RII_end_filter_2d = &wrfsi_utils::compute_time ($RII_end_filter, -6);
my $RII_pattern_2d = "flx\.ft06";
my $yyyy = 0;
my $foundRII = 0;

  foreach (@allfiles) {
    if (/(\d\d\d\d)(\d\d\d\d\d\d)\.grib$/) {
      $foundRII = 1;
      my $yyyy = $1; my $mmddhh = $2;
      my $this_time = $yyyy.$mmddhh;
      if (/^$RII_pattern_2d/) {
          if ( ($this_time >= $RII_start_filter_2d) &&
               ($this_time <= $RII_end_filter_2d) ) {
               @gribfiles = (@gribfiles,$_);
	  }
      }
      else {
          if ( ($this_time >= $RII_start_filter) &&
               ($this_time <= $RII_end_filter) ) {
               @gribfiles = (@gribfiles,$_);
          }
      }
    }
  ...
  if ($foundRII == 1) {
    if ($ngribfiles == 0) {
      my $msg = "FAILURE: Found RII-type files but nothing during $RII_start_filter - $RII_end_filter";
      print "$msg\n";
      print LOG "$msg\n";
      #exit;
    }
    else {
      my $msg = "Found RII-type files during $RII_start_filter - $RII_end_filter";
      print "$msg\n";
      print LOG "$msg\n";
    }
  }
...
my $foundFSL = 0;
  ...
  if ($foundFSL == 1) {
    if ($ngribfiles == 0) {
      ...
      my $msg = "Found FSL-type files in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
    }
  }
...
my $foundNCEP = 0;
  ...
  if ($foundNCEP == 1) {
    if ($ngribfiles == 0) {
      my $msg = "FAILURE: Found NCEP1-type files but nothing in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
      #exit;
    }
    else {
      my $msg = "Found NCEP1-type files in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
    }
  }
...
my $foundNCEP = 0;
  ...
  if ($foundNCEP == 1) {
    if ($ngribfiles == 0) {
      my $msg = "FAILURE: Found NCEP2-type files but nothing in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
      #exit;
    }
    else {
      my $msg = "Found NCEP2-type files in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
    }
  }
...
my $foundNCEP = 0;
  ...
  if ($foundNCEP == 1) {
    if ($ngribfiles == 0) {
      my $msg = "FAILURE: Found NCEP3-type files but nothing in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
      #exit;
    }
    else {
      my $msg = "Found NCEP3-type files in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
    }
  }
...
my $foundAFWA = 0;
  ...
  if ($foundAFWA == 1) {
    if ($ngribfiles == 0) {
      my $msg = "FAILURE: Found AFWA-type files but nothing in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
      #exit;
    }
    else {
      my $msg = "Found AFWA-type files in the specified time window.";
      print "$msg\n";
      print LOG "$msg\n";
    }
  }
