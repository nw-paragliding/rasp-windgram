...
#Nested window RASP run for TIGER
if $WXTOFLY_RUN/run_rasp_nested.sh "PNW${N}" "TIGER${N}" "$WXTOFLY_RUN/PARAMETERS/rasp.run.parameters.TIGER${N}.${INIT}z" ; 
then
	$WXTOFLY_RUN/run_background_task.sh $WXTOFLY_RUN/run_tasks_w2.sh "TIGER${N}"
else
