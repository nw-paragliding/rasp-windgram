...
if(0){
  ...
  @pInfo_h_grid=$h_grid->packInfo();
    ...
    $nxdim_lab=$h_grid->Label( -text => "Horizontal Dimension X:", -fg => $colorN) 
                   ->grid( -row => ++$rrow, 
                           -column => 1, 
                           -sticky => 'w');
   ...
   my $nx=$h_grid->Frame( -highlightthickness => 0) 
    ...
    $nydim_lab=$h_grid->Label( -text => "Horizontal Dimension Y:", -fg => $colorN) 
                   ->grid( -row => ++$rrow, 
                           -column => 1, 
                           -sticky => 'w');
...
sub map_utils_setup_nest {
   ...
   if ($my_catch eq "") { 
     warn("There is a major problem with the namelist file: $domain_nl.\n"), return(1);
   }
...
sub write_nest_table {
    ...
    if(-e $nest_info) { unlink $nest_info; }

    # If nothing to do, return.
    if ($num_nests <=0 ) { return;}
...
sub access_to_nest_tabs {
   ...
   if ($globalMap or $projection_type eq 'RL') { 
...
sub reinstate_gen_map_vars{
     ...
     $pix_per_gpoint=$pix_per_gpoint_HOLD;
     adjust_bbox();
     update_tags();
...
sub set_lat2_lon_wdgt_state{
  ...
  if ( $projection_type eq 'LC' && $grid_val_restrict == 0) {
...
sub check_domain_args {
    ...
    # RAR - The NY Dimension in the NMM MUST be odd
    #
    if ($projection_type eq 'RL' && !($ny_dim % 2)) {
        $msg="Horiz Y must be an odd number value for NMM. 
Press 'Yes' to increase by 1 and 'No' to decrease by 1.";
        my $disp = yesno_dbox("Horizontal Dim Y", $msg, "Yes"); 
        if($disp eq 'Yes') {
          $ny_dim ++ unless $ny_dim % 2;
        } else {
          $ny_dim -- unless $ny_dim % 2;
        }
    }
...
sub suggest_proj_type {
    ...
    } elsif (abs ($grid_cen_lat_cmn) <= 23.5) {
...
sub render_new_map {
   ...
   # Insist that standard lat/lon equal center lat/lon for NMM, only.
   if ($nmm) { set_center_latlon(); }
...
# ---
   ...
   $pix_per_gpoint_HOLD=$pix_per_gpoint=($RR-$LL)/$nx_dim;
...
# Set the internally used grid spacing var for the NMM core 
# if rotlat (RL) is selected.
sub set_dx {

   if ($grid_spacing_dkm <= 0.009 && $projection_type eq 'RL' ) { set_disp_dx(); }
...
# Set the externally displayed grid spacing var for the NMM core 
# if rotlat (RL) is selected.
sub set_disp_dx {
...
# Set the internally used x-dimension var for the NMM core 
# if rotlat (RL) is selected.
sub set_nx {
...
# Set the externally displayed x-dimension var for the NMM core 
# if rotlat (RL) is selected.
sub set_disp_nx {
   if ($projection_type eq 'RL') {
