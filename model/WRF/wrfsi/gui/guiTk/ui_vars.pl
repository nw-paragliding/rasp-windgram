...
#  $mw->optionAdd(qw/Idt*canvas.width:   400/);
  $mw->optionAdd(qw/Idt*canvas.width:   1000/);
#  $mw->optionAdd(qw/Idt*canvas.height:  200/);
  $mw->optionAdd(qw/Idt*canvas.height:  500/);
...
# ---------------------------------------
# assign_nl_vars_nmm 
#     Replacement line from WRF web site: my $x= 111.2*($nl_var{MOAD_DELTA_X}[0]**2. + (.9838*$nl_var{MOAD_DELTA_Y}[0])**2.)**0.5;
...
sub assign_nl_vars_nmm {
     ...
     ##  original line  my $x=$nl_var{MOAD_DELTA_X}[0] / 0.006500;
     my $x= 111.2*($nl_var{MOAD_DELTA_X}[0]**2. + (.9838*$nl_var{MOAD_DELTA_Y}[0])**2.)**0.5;
     $x=sprintf ("%.1f",$x);
