...
Red mark indicates localization files predate ver 2.0.");
...
sub load_domainSelection {
   ...
   if($domain_lb->info('selection') eq "") { return(1); }
...
sub load_domain {
   ...
   $d_num="d01";
   $static_select=
      "$dataroot_select/$domain_select/static/static.$window_domain_arg.$d_num";
...
See documentation on parameters SILAVWT_PARM_WRF and TOPTWVL_PARM_WRF.";
      ...
      update_force_loc_mb();
      write_namelist();
      $hint_msg="Press 'Localize' if you are ready to run gridgen_model and process geographical data.";
