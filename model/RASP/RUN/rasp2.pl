...
### GRIB FILENAMES
  ...
  ## 13km FSL RUC2 @ ~20Mb/file out to f18 hrs on server on NCEP server ftp.ncep.noaa.gov  AKA ftpprd.ncep.noaa.gov AKA 140.90.33.31
  ##     ruc2a.tiiz.pgrb13anl.girb2  then  ruc2a.tiiz.pgrb13fhh.girb2  for subsequent files.
          ...
          for ( $idomain=2 ; $idomain<=$MAXDOMAIN{$regionname}[$IWINDOW] ; $idomain++ )
          {
            ...
            ( $endyyyy4dom[$idomain],$endmm4dom[$idomain],$enddd4dom[$idomain],$endhh4dom[$idomain], $min2 ) = &GMT_plus_mins( $startyyyy4dom[$idomain],$startmm4dom[$idomain],$startdd4dom[$idomain],$starthh4dom[$idomain], 0, (60*$FORECAST_PERIODHRS{$regionname}[$IWINDOW]+$enddeltamins) ); 
              ...
              if ($LMODELINIT>0)
              {
                ...
                ### IF LATER CHANGE TO DOING A GROUP grib_prep, DO IT HERE    tjo  this is the place.  condition on existing_parent or -w
...
#                  if ($LPRINT>1) { print $PRINTFH "  using $existing_parent-WINDOW/log/$run_yyyymmddhh. to check previous runs existing_parent for $JOBARG    \n"; }
                ...
                ### test for wrfprep run errors  tjo  this required some copying from parent-window to nest-window  above for when -w is used
		### could instead have a conditional here for which log to check, if multinest then check parent   
...
### SET  FTP PARAMETERS
{
  ...
  elsif ( $GRIBFILE_MODEL eq 'RUCH' )
  {
    ...
    ### if change gribftpsite(s) also need changes below and in gribftpget
    ### *NB* FILENAMES MUST BE SAME AT ALTERNATE SITE $gribftpsite2
      ### but the publicly available RUC at ftpprd.ncep.noaa.gov does use  diretories
      ### $gribftpsite1 = 'ftpprd.ncep.noaa.gov';
      ### $gribftpdirectory0 = "/pub/data/nccf/com/ruc/prod/" ;
      ### $gribftpdirectory[1] = "ruc2a.yyyymmdd/" ;
      ### $gribftpsiteid1 = 'RUC2';
    $gribftpsite1 = 'ftpprd.ncep.noaa.gov';
    $gribftpsiteid1 = 'RUC2';
    $gribftpsite2 = '';
